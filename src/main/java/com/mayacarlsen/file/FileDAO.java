...
public class FileDAO {
    ...
    public static final String SELECT_THUMBNAIL_SQL = "SELECT F.FILE_ID, F.USER_ID, F.FILE_NAME, F.FILE_TYPE, F.FILE_TITLE, F.THUMBNAIL, F.PUBLISH_FILE, F.CREATE_DTTM, F.UPDATE_DTTM, "
            + "U.FIRST_NAME, U.LAST_NAME " 
            + "FROM FILES F, USERS U "
            + "WHERE F.user_id = U.user_id AND F.file_id = :file_id";

    public static final String SELECT_THUMBNAIL_WHERE_SQL = " AND F.publish_file = :publish_file";
    ...
    public static File getPublishedFile(Integer fileId) {
        try (Connection conn = sql2o.open()) {
            return conn.createQuery(SELECT_PUBLISHED_FILE_SQL)
    ...
    public static File getThumbnail(Integer fileId) {
        try (Connection conn = sql2o.open()) {
            return conn.createQuery(SELECT_THUMBNAIL_SQL)
                    .addParameter("file_id", fileId)
    ...
    public static File getThumbnail(Integer fileId, Boolean publisheFile) {
        try (Connection conn = sql2o.open()) {
	    return conn.createQuery(SELECT_THUMBNAIL_SQL + SELECT_THUMBNAIL_WHERE_SQL)
		    .addParameter("file_id", fileId)
		    .addParameter("publish_file", publisheFile)
		    .executeAndFetchFirst(File.class);
	} catch (Exception e) {
	    throw new RuntimeException(e);
	}
    ...
    public static Integer createFile(File file) {
        try (Connection conn = sql2o.beginTransaction()) {
            Integer fileId = (Integer) conn.createQuery(INSERT_FILE_SQL, true)
                    ...
                    .addParameter("thumbnail", file.getThumbnail())
    ...
    public static Integer updateFile(File file) {
        try (Connection conn = sql2o.beginTransaction()) {
            int count = conn.createQuery(UPDATE_FILE_SQL)
                    .addParameter("user_id", file.getUser_id())
                    .addParameter("file_name", file.getFile_name())
                    .addParameter("file_type", file.getFile_type())
                    .addParameter("file_title", file.getFile_title())
                    ...
                    .addParameter("file", file.getFile())
                    .addParameter("publish_file", file.getPublish_file())
                    .addParameter("file_id", file.getFile_id())
